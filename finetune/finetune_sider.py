...
def parse_args():
    ...
    parser.add_argument('--lr0', default=5e-5, type=float, help="learning rate of encoder0")
    parser.add_argument('--lr1', default=5e-5, type=float, help="learning rate of encoder1")
...
def main(args,start_time,output_dir):
    ...
    for epoch in range(args.num_epochs):
        ...
        early_stop_cnt += 1
        trainAll(modelAll,train_loader,optimizer2,device)
        # train(model, train_loader, optimizer, device)
        # record
        train_loss, train_roc = 0, 0
        train_loss, train_roc = testAll(modelAll, train_loader, device)
        record[0].append(train_loss)
        record[1].append(train_roc)
        valid_loss, valid_roc = testAll(modelAll, valid_loader, device)
        record[2].append(valid_loss)
        record[3].append(valid_roc)
        test_loss, test_roc = testAll(modelAll, test_loader, device)
        ...
        train_rocList.append(train_roc)
        test_rocList.append(test_roc)
        # update model
        if test_roc > best[2]:
    ...
    _, auc_roc = testAll(modelAll, train_loader, device)
    logger.info(f"AUC_ROC: {auc_roc :.8f}")
    logger.info("Valid set:")
    _, auc_roc = testAll(modelAll, valid_loader, device)
    logger.info(f"AUC_ROC: {auc_roc :.8f}")
    logger.info("Test set:")
    _, auc_roc = testAll(modelAll, test_loader, device)
    logger.info(f"AUC_ROC: {auc_roc :.8f}")
    logger.info(f"train AUC_ROC List: {train_rocList}")
    logger.info(f"test AUC_ROC List: {test_rocList}")
    logger.info("\n=======Finish=======")    
